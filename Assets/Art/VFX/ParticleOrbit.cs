...
public class ParticleOrbit : MonoBehaviour {
	...
	void Start()
	{
		...
		if (m_System.main.simulationSpace == ParticleSystemSimulationSpace.Local && CenterOfGravity != null)
		{
	...
	private void LateUpdate()
	{
		...
		for (int i = 0; i < numParticlesAlive; i++)
		{
			...
			uint seed = m_Particles[i].randomSeed % 2;
			int LR = (seed > 0 )? 1 : -1;
			if (m_System.main.simulationSpace == ParticleSystemSimulationSpace.World)
			{
				gravitation = CenterOfGravity.position - m_Particles[i].position;
				gravitation = Vector3.Cross(Vector3.down * LR, gravitation);
			}
			else
			{
			...
			m_Particles[i].velocity *= 1 - dampening;
		}
